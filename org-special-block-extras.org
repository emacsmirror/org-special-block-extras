...
(load-file "./tests/e2e.el")
(org-html-export-to-html)

;; Run the next line whenever a yaml file changes.
...
# TODO: The main difference between “doc” and “margin” is that the latter admits a block form,
...
# TODO: The main difference between “doc” and “margin” is that the latter admits a block form,
# whereas the former only admits a link form. We can merge the two into a single type.
# For example, “[[doc: !Here's more explanatory prose]]” ≈ “[[margin: Here's more explanatory prose]]”; i.e.,
# doc links that start with ‘!’ are considered to be inline literals and not addresses to look-up information.
...
# TODO: Favour plists over HTML tags:
#
#   ;; dom.el has multiple methods to maniuplate HTML DOM plist values.
#   (shr-dom-to-xml '(name ((key₀ . value₀) (key₁ . value₁)) "contents"))
# ⇒ "<name key₀=\"value₀\" key₁=\"value₁\">contents</name>"
...
** See changes for current buffer only
C-c M-g will pop up a menu just for that fie and you can hit d there to get a diff, then stage as you like!
...
---[[doc:org-defblock][defblock]]--- that lets users treat 𝒳 as a string-valued function in the style
of doc:defun.
...
BACKEND is the export back-end being used, as a symbol."
  ...
  (cl-loop for blk in org--supported-blocks
	  ...
	  ;; TODO: Make the `main-arg' just be the first keyword arg. That is “begin_X main-value :keyₖ value-k” ≈ “begin_X :key₀ main-value :keyₖ value-k”.
	  ;; This will simplify cases where this is no sense of a main-arg, such as org-demo, whose arg list starts with “nil nil” since there's no main-arg and its default is nil.
	  ;; One consequence of this approach is that the first keyword arg is special, it's key can be omitted when it occurs first: “begin_X main-value” ≈ “begin_X :key₀ main-value”.
	  (-let [val (eval `(,(intern (format "org-block/%s" blk))
	    ...
	    (when (or (equal blk 'margin) (equal blk 'tooltip)) (setq val (format "@@html: %s@@" (s-replace "@" "＠" (s-chop-prefix "#+begin_export html \n" (s-chop-suffix "\n#+end_export" val))))))
            (insert val))
...
(deftest "Constant blocks preserve indentation/enumeration"
  ...
  :tags '(core)
  (org-defblock go nil "doc" "hello") ;; Constantly “hello”
  (should (equal
"
  1. item one
  2. item two
...
- badge:Hello|World Use *badge-style links* to embed GitHub stars, Reddit subs, or custom  project metadata via =shields.io=.
...
# - *🧩 Compose blocks modularly* --- nest and thread blocks functionally, just like  Lisp =funcall=.
# TODO. FIXME. - Embrace *modularity*: Build new blocks from existing ones with  =thread-blockcall= --- like Lisp =funcall=.
 
   # With a single macro =defblock=, this package provides *dozens of
   # built-in Org-mode blocks*, all tailored for rich export and seamless
   # Emacs display:
 ...
 *🧱 Ready-to-Use Custom Blocks*
  ...
  # | =documentation=      | Define glossary/tooltips --- enrich your articles with inline help.                   |
  # | =remark= / =rremark= | Editorial comments --- provide in-place or top-level reviewer feedback.               |
...
*Ready-to-Use Custom Links*
    | Link               | Purpose                                                                                 |
    |--------------------+-----------------------------------------------------------------------------------------|
    | =doc:label=          | Glossary/dictionary tooltips ---link to local or global explanations; e.g., doc:cl-loop |
...
+ 1️⃣ Anyone who wants to keep Org-mode as the single source of truth for pink:rich
  exports
...
*** TODO COMMENT Extra notes, either make into yaml tests or more to an FAQ section
...
~(-reduce #'/ (number-sequence 1.0 𝓃))~ compute when given a whole
number 𝓃?
...
** Nice Keystroke Renditions: kbd:C-h_t
  :PROPERTIES:
...
- ~kbd:𝒳~ will show a tooltip defining 𝒳, as an Emacs Lisp function, if possible.
  For example, ~kbd:C-h_t~ is kbd:C-h_t; and likewise =<kbd:M-s h .>= is <kbd:M-s h
...
#+end_src
#+end_details
#+begin_details "★ Example: Lengthy ‘remark’ with multiple code blocks!" :title-color orange
It's a
#+begin_tooltip simple
...
** Intro, motivating examples                                        :ignore:
   ...
   Sometimes I want to use the same [[doc:org-block/tooltip][tooltip]] multiple times, so I write
   the tooltip content once in a [[org-block/documentation][documentation]] block and then
   reference it using the the [[doc:org-block/doc][doc]] link type.

   # For instance,
   
   #+begin_documentation Category Theory :label "cat"
   A theory of typed composition; e.g., typed monoids, such as matrices and matrix multiplication.
   #+end_documentation

   Now I can reference this documentation entry using its label as in
   doc:cat or using its full entry name, with spaces replaced by
   underscores, as in doc:Category_Theory; finally, we can also use
   descriptions as in [[doc:cat][graphs + monoids]]. 

   The ~:label~ is optional; when not provided it defaults to the name of
   the entry with spaces replaced by ‘_’.  For more, see
   doc:org-block/documentation.  Entry names do not need to be unique,
   but labels do!  ( The labels are like the addresses used to look up a
   person's home. )
      
   In this way, I can create a custom glossary.
   ...
   Moreover, the  [[doc:org-block/doc][doc]] link type lets me reference Emacs-documentation such as
   doc:loop or doc:thread-last, and English-dictionary entries such as
   doc:happiness.

   Finally, *in Emacs*, these links also have tooltips ☺️
   ...
   utility.
   TODO: Make a method docs-update to update the docs database, regardless of
   export. Low priority. THIS EXISTS AS (org-docs-load-libraries) ??
...
* COMMENT How Do I make my own block and link types?
...
# TODO: The docstring should also mention “org-parse” and its use with “raw-contents”?
